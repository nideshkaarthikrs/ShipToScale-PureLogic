...
function ReasoningCard({ precedents }) {
  ...
  return (
    <div className="mt-2 rounded-xl border border-indigo-100 bg-indigo-50/70 px-3 py-2 text-xs text-indigo-900">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
...
export default function ChatPanel({ analysis, suggestedChips }) {
  ...
  return (
    <section className="surface-card flex h-[600px] flex-col overflow-hidden">
      <header className="flex items-start gap-3 border-b border-slate-200 px-5 py-4">
        ...
        <div className="flex-1">
          <h2 className="text-sm font-black text-slate-950">Ask about this document</h2>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {chips.map((q) => (
                <button
                  ...
                  onClick={() => send(q)}
                  disabled={streaming}
                  className="rounded-full border border-slate-200 bg-white/80 px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-emerald-300 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {q}
                </button>
              ))}
              ...
              <li key={i} className="flex items-start gap-3">
                <div
                  className={`mt-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full ${
                    m.role === 'user' ? 'bg-slate-950 text-white' : 'bg-emerald-100 text-emerald-700'
                  }`}
                >
      ...
      >
        <input
          ...
          disabled={streaming}
          className="flex-1 rounded-xl border border-slate-300 bg-white/90 px-3 py-2 text-sm shadow-inner focus:border-slate-500 focus:outline-none disabled:bg-slate-50"
        />
        {streaming ? (
          <button
            type="button"
            onClick={stopStreaming}
            className="inline-flex items-center gap-1.5 rounded-xl bg-rose-600 px-3 py-2 text-sm font-semibold text-white hover:bg-rose-700"
          >
            Stop
          </button>
        ) : (
          <button
            type="submit"
            disabled={!input.trim()}
            className="inline-flex items-center gap-1.5 rounded-xl bg-slate-950 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
          >
