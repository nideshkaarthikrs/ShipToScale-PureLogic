  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold text-slate-900">Winning Legal Arguments</h2>
      ...
      <ol className="mt-6 space-y-4">
        ...
        {args.map((a, idx) => (
          <li key={a.id} className="rounded-xl border border-slate-200 p-4 transition hover:border-slate-300">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <div className="flex items-baseline gap-2">
                <span className="text-xs font-bold text-slate-400">#{idx + 1}</span>
                <h3 className="text-sm font-semibold text-slate-900">{a.headline}</h3>
              </div>
